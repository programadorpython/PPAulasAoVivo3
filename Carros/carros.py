...
class CarroHibrido(Carro):
    """
    Modelar um carro híbrido herdando a class pai Carro
    """

    def __init__(self, modelo_carro, fabricante):
        """
        iniciliaza os atributos da classe filha (CarroHibrido)
        :param modelo_carro: string
        :param fabricante: string
        """
        super().__init__(modelo_carro, fabricante)  # Aqui traz da classe-pai os atributos
        self.opcionais = []  # Atributo exclusivo somente da classe filha

    def mostrar_opcionais(self):
        """
        Percorre a lista e mostra os opcicionais do objeto da classe-filha
        :return:
        """
        print(f"Esse {self.fabricante} {self.modelo_carro} é híbrido e possui os opcionais: ")
